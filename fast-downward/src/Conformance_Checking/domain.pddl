...
;;
;;EXISTENCE-task1 - (FINALLY task1) - An instance of the task task1 must occur at least once in the trace.
;;

(:derived (existence-task1)
(exists (?t - task1) (traced ?t)))

;;
;;ABSENCE-task2 - (NOT (FINALLY task2)) - task2 never occur in the trace.
;;

(:derived (absence-task2)
(not (exists (?t - task2) (traced ?t))))
